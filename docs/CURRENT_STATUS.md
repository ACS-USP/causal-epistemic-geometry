...
**Scientific stage:** `Q2_DEVELOPMENT_V2_COMMON_PANEL_COLLECTION_AUTHORIZED`
...
- RunPod: `A40_PROVISIONING_AUTHORIZED`
- GPU work authorized: `true`
- New scientific experiment authorized: `q2_v2_frozen_common_panel_continuation`
...
|---|---|---|
| `Q2_CONTROLLER_HELDOUT_GEOMETRY_V2` | `FROZEN_COMMON_PANEL_COLLECTION_AUTHORIZED` | Six of six conceptual source axes qualified, and label-free per-direction calibration produced a qualified bank of 24 meaningful controllers across six families, including 12 causal directions and four dose bins. Four fresh nulls are SVD-span-orthogonal. The original US$25 gate correctly stopped collection. Under the prospective US$45 amendment, an exact A40 engine benchmark projected US$36.1105 cumulatively, while RTX 6000 Ada failed exact discrete equivalence. The hard ceiling passes, but the principal-reported US$5.42 wallet was initially insufficient for the US$32.1623 common-panel projection. A subsequent US$30 principal top-up removed the financial block while the common panel remained at zero rows. The exact frozen 6,960-row panel is now authorized on the A40 serial reference engine; predictive M0/M1/M2 remain unrun until collection is complete. |
...
- `geometry_q2`: `Q2_V2_FROZEN_COMMON_PANEL_COLLECTION_AUTHORIZED`
- `committee_q3`: `NOT_RUN`
- `confirmatory_holdout`: `CONSUMED_CONFIRMATORY_CLOSED`
- `model_downloads_local`: `FORBIDDEN`
- `current_task`: `Q2_V2_FROZEN_COMMON_PANEL_COLLECTION`
- `free_generation`: `NOT_AUTHORIZED; ONLY_FROZEN_6960_ROW_Q2_V2_PANEL`
