...
2. Q1 smoke uses at most 20 new items and one independent seed per candidate.
3. Only a non-obviously unusable candidate may advance to Q2: 50 new items and two
   independent seeds. The Q2 fixed cap is chosen before its outcomes are observed.
4. The campaign stops after Q2 and reports either
...
- a generous fixed cap of 2048 new tokens; truncation is diagnostic, not the
  desired source of difficulty
...
python scripts/run_external_qualification.py \
  --candidate CRUXEval \
  --data /workspace/causal-epistemic-geometry/data/cruxeval_q1.jsonl \
  --stage q1_smoke \
...
review the Q2 bundle before any future steering pilot is designed.
