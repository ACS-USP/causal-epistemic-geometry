...
import numpy as np
import pytest
from scripts.run_finalization_timing_collection import (
    LockValidationError,
    execute_collection,
...
def _fixture(tmp_path: Path) -> Path:
    ...
    schedule = [{"schedule": "fixture", "cap": 4096}]
...
def test_default_mode_remains_validation_only(tmp_path: Path, monkeypatch, capsys) -> None:
    lock_path = _fixture(tmp_path)
    monkeypatch.setattr(
        "scripts.run_finalization_timing_collection._default_backend_factory",
