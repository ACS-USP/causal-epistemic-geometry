...
from .finalization_timing import NAMESPACE
from .rollouts import RolloutRecord

JOURNAL_VERSION = "q1-v3-finalization-timing-journal-v1"


def identity_hash(identity: Mapping[str, Any]) -> str:
    """Hash the caller supplied identity without dropping any fields."""

    return stable_digest("Q1-V3-FINALIZATION-TIMING-JOURNAL-IDENTITY", canonical_json(identity))
...
class FinalizationTimingJournal:
    ...
    def _validate_entry(self, entry: Mapping[str, Any]) -> None:
        ...
        if not isinstance(schedule, Mapping) or not isinstance(record, Mapping):
            raise ValueError("finalization timing journal entry is malformed")
        self._validate_schedule(schedule)
    ...
    def _validate_wrapper(self, wrapped: Any) -> dict[str, Any]:
        ...
        if wrapped.get("journal_version") != JOURNAL_VERSION:
            raise ValueError("unsupported finalization timing journal version")
        if wrapped.get("identity_hash") != self.identity_digest:
            raise ValueError("finalization timing journal provenance does not match this run")
        if wrapped.get("identity") != self.identity:
            raise ValueError("finalization timing journal identity does not match this run")
        schedule = wrapped.get("schedule")
        record = wrapped.get("record")
        if not isinstance(schedule, dict) or not isinstance(record, dict):
            raise ValueError("finalization timing journal schedule and record must be objects")
        key = physical_key(schedule)
        if wrapped.get("physical_key") != list(key):
            raise ValueError("finalization timing journal physical key mismatch")
        if wrapped.get("schedule_identity_hash") != schedule.get("schedule_identity_hash"):
            raise ValueError("finalization timing journal schedule identity mismatch")
        self._validate_schedule(schedule)
        self._validate_record(schedule, record)
...
FinalizationTimingRolloutJournal = FinalizationTimingJournal

__all__ = [
    "FinalizationTimingJournal",
    "FinalizationTimingRolloutJournal",
    "JOURNAL_VERSION",
    "identity_hash",
    "physical_key",
    "schedule_key",
