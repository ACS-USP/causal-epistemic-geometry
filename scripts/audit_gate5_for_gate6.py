...
def compare_maps(
    actual: Any, expected: Any, prefix: str, differences: list[dict[str, Any]]
) -> None:
    ...
    if isinstance(actual, list) and isinstance(expected, list):
        if len(actual) != len(expected):
            differences.append(
                {"field": prefix, "actual_length": len(actual), "expected_length": len(expected)}
            )
...
def classify_manipulation(metrics: dict[str, dict[str, float]]) -> bool:
    random_mean = np.mean(
        [metrics[f"SUSTAINED_RANDOM_R{i}"]["semantic_change_rate"] for i in range(4)]
    )
...
def classify_gate5(
    estimates: dict[str, dict[str, float]], manipulation_pass: bool, engineering_pass: bool
) -> str:
    ...
    for sign in ("PLUS", "MINUS"):
        ...
        guard = (
            ...
            and current["validity"] >= estimates["BASELINE"]["validity"] - 0.05
        )
        ...
        movement[name] = bool(
            ...
            and competence
            and current["D"] >= 0.05
            and current["D"] - np.mean(random_d) >= 0.05
        ...
        useful[name] = bool(
            ...
            and current["G"] >= 0.03
            and current["C"] >= 0.03
            and current["C"] - np.mean(random_c) >= 0.05
    ...
    if all(
        not (
            estimates[f"SUSTAINED_{s}"]["validity"] >= 0.90
            and estimates[f"SUSTAINED_{s}"]["validity"] >= estimates["BASELINE"]["validity"] - 0.05
            and estimates[f"SUSTAINED_{s}"]["accuracy"] >= estimates["BASELINE"]["accuracy"] - 0.10
        )
        for s in ("PLUS", "MINUS")
    ):
        return "GATE5_SUSTAINED_DESTRUCTIVE"
    if any(
        estimates[f"SUSTAINED_{s}"]["D"] - estimates[f"ONE_SHOT_{s}"]["D"] >= 0.03
        for s in ("PLUS", "MINUS")
    ):
...
def main() -> int:
    ...
    for phase, (n_items, conditions, rollouts) in PHASE_SPECS.items():
        ...
        if len(phase_ids[phase]) != n_items or missing or extra:
            schedule_errors.append(
                f"{phase}: ids={len(phase_ids[phase])}, expected={n_items}, missing={len(missing)}, extra={len(extra)}"
            )
        schedule[phase] = {
            "observed_rows": len(phase_rows),
            "expected_rows": n_items * len(conditions) * len(rollouts),
            "items": len(phase_ids[phase]),
            "missing": missing,
            "extra": extra,
        }
    ...
    historical_overlap = (
        sorted(set.union(*(phase_ids.values())) & historical_ids) if phase_ids else []
    )
    lock_ids = {
        phase: set(map(str, values)) for phase, values in lock["fresh_splits"]["ids"].items()
    }
    manifest_mismatches = {
        phase: sorted(phase_ids.get(phase, set()) ^ values) for phase, values in lock_ids.items()
    }
    ...
    for row in rows:
        ...
        if failed:
            provenance_errors.append(
                f"{row.get('phase')}:{row.get('item_id')}:{row.get('condition')} failed {failed}"
            )
        ...
        should_be = (
            ...
            else ("one_shot" if condition.startswith("ONE_SHOT") else "sustained")
        )
        if duration != should_be:
            provenance_errors.append(
                f"{row.get('phase')}:{condition} duration={duration}, expected={should_be}"
            )
        if condition in SOURCE_CONDITIONS or condition == "BASELINE":
            ...
            expected_meta = (
                ...
                ALPHA,
                LAYER,
                "1304d6fc8dd0985895bc802885b156bc9be49d1afc58d00b013f51830cf9b9df",
            )
        ...
        observed_meta = (
            duration,
            float(stop.get("intervention_alpha", 0.0)),
            stop.get("intervention_layer"),
            stop.get("intervention_vector_hash"),
        )
        if not (
            observed_meta[0] == expected_meta[0]
            and math.isclose(observed_meta[1], expected_meta[1], abs_tol=1e-12)
            and observed_meta[2:] == expected_meta[2:]
        ):
            provenance_errors.append(
                f"{row.get('phase')}:{condition} intervention metadata={observed_meta}, expected={expected_meta}"
            )
        metadata_rows.append(
            {
                ...
                "condition": condition,
                "duration": duration,
                "forward_count": stop.get("intervention_forward_count"),
                "prefill_applications": stop.get("intervention_prefill_applications"),
                "decode_applications": stop.get("intervention_decode_applications"),
                "vector_hash": stop.get("intervention_vector_hash"),
            }
        )
    ...
    for phase, phase_keyed in keyed.items():
        if phase == "SUSTAINED_MANIPULATION":
            for item in phase_ids[phase]:
                values = {
                    phase_keyed[(item, condition, 0)]["seed"] for condition in PRIMARY_CONDITIONS
                }
                if len(values) != 1:
            ...
            for item in phase_ids[phase]:
                for rollout in (0, 1):
                    values = [
                        phase_keyed[(item, condition, rollout)]["seed"]
                        for condition in (
                            SOURCE_CONDITIONS if phase == "SOURCE_CHECK" else PRIMARY_CONDITIONS
                        )
                    ]
                    if len(values) != len(set(values)):
                        seed_errors.append(
                            f"{phase}:{item}:rollout={rollout} condition seeds are not independent"
                        )
    retry_fields = [
        key(row)
        for row in rows
        if row.get("retry") or row.get("retry_provenance") or row.get("attempt") not in (None, 0, 1)
    ]

    # Source metrics from raw status/answers.
    source_keyed = keyed["SOURCE_CHECK"]
    source_outcomes: dict[str, dict[str, list[str]]] = {
        condition: {} for condition in SOURCE_CONDITIONS
    }
    ...
    for item in source_items:
        for condition in SOURCE_CONDITIONS:
            ...
            source_tokens[condition].extend(
                int(row.get("generated_token_count", 0)) for row in values
            )
            source_correct[condition].extend(
                str(row.get("status")) == "VALID_CORRECT" for row in values
            )
            source_valid[condition].extend(str(row.get("status")) in VALID for row in values)
    cross = [
        left != right
        for item in source_items
        for left in source_outcomes["CAREFUL"][item]
        for right in source_outcomes["DIRECT"][item]
    ]
    within_careful = [
        source_outcomes["CAREFUL"][item][0] != source_outcomes["CAREFUL"][item][1]
        for item in source_items
    ]
    within_direct = [
        source_outcomes["DIRECT"][item][0] != source_outcomes["DIRECT"][item][1]
        for item in source_items
    ]
    source_metrics = {
    ...
    source_class = classify_source(
        {
            "careful_validity": source_metrics["CAREFUL_validity"],
            "direct_validity": source_metrics["DIRECT_validity"],
            "X": source_metrics["X"],
            "S": source_metrics["S"],
            "careful_mean_tokens": source_metrics["CAREFUL_mean_tokens"],
            "direct_mean_tokens": source_metrics["DIRECT_mean_tokens"],
            "careful_median_tokens": source_metrics["CAREFUL_median_tokens"],
            "direct_median_tokens": source_metrics["DIRECT_median_tokens"],
        }
    )
    ...
    def phase_metrics(
        ...
        for condition in conditions:
            matrix = np.asarray(
                [
                    [error(phase_keyed[(item, condition, rollout)]) for rollout in range(rollouts)]
                    for item in items
                ],
                dtype=bool,
            )
            ...
            selected = [
                phase_keyed[(item, condition, rollout)]
                for item in items
            ...
            summaries[condition] = {
                "n": len(selected),
                "validity": float(np.mean([str(row.get("status")) in VALID for row in selected])),
                "accuracy": float(
                    np.mean([str(row.get("status")) == "VALID_CORRECT" for row in selected])
                ),
                "status_counts": dict(statuses),
                "mean_tokens": float(
                    np.mean([int(row.get("generated_token_count", 0)) for row in selected])
                ),
            }
    ...
    for condition in PRIMARY_CONDITIONS[1:]:
        ...
        for item in manip_items:
            source_row = keyed["SUSTAINED_MANIPULATION"][(item, "BASELINE", 0)]
            ...
            raw_changes.append(
                list(source_row.get("generated_token_ids", []))
                != list(current.get("generated_token_ids", []))
            )
    ...
    random_rates = [
        manip_metrics[f"SUSTAINED_RANDOM_R{i}"]["semantic_change_rate"] for i in range(4)
    ]
    manip_metrics["RANDOM_SUMMARY"] = {
        "semantic_change_rate_mean": float(np.mean(random_rates)),
        "semantic_change_rate_max": float(np.max(random_rates)),
        "raw_sequence_change_rate_mean": float(
            np.mean(
                [
                    manip_metrics[f"SUSTAINED_RANDOM_R{i}"]["raw_sequence_change_rate"]
                    for i in range(4)
                ]
            )
        ),
    ...
    for sign in ("PLUS", "MINUS"):
        for metric in ("semantic_change_rate", "raw_sequence_change_rate"):
            manip_metrics[f"CONTRAST_{sign}"] = {
                "sustained_minus_one_shot_" + metric: manip_metrics[f"SUSTAINED_{sign}"][metric]
                - manip_metrics[f"ONE_SHOT_{sign}"][metric],
                "sustained_minus_random_mean_" + metric: manip_metrics[f"SUSTAINED_{sign}"][metric]
                - manip_metrics["RANDOM_SUMMARY"][metric + "_mean"],
                "sustained_minus_random_max_" + metric: manip_metrics[f"SUSTAINED_{sign}"][metric]
    ...
    for condition in PRIMARY_CONDITIONS:
        if condition == "BASELINE":
            eval_estimates[condition] = {
                "validity": eval_summaries[condition]["validity"],
                "accuracy": eval_summaries[condition]["accuracy"],
            }
        else:
            result = independent_estimands(
                eval_matrices["BASELINE"].astype(float), eval_matrices[condition].astype(float)
            )
            eval_estimates[condition] = {
                **result,
                "validity": eval_summaries[condition]["validity"],
        ...
        metric_rows.append(
            {
                ...
                "recomputed": actual,
                "stored": expected,
                "match": exact_or_close(actual, expected),
            }
        )
    for condition, estimate in eval_estimates.items():
        if condition not in stored_eval["evaluation"]:
            comparisons.append(
                {"field": f"evaluation.{condition}", "actual": estimate, "expected": None}
            )
            continue
        for metric, actual in estimate.items():
            ...
            metric_rows.append(
                {
                    "metric": f"evaluation.{condition}.{metric}",
                    "recomputed": actual,
                    "stored": expected,
                    "match": exact_or_close(actual, expected),
                }
    ...
    metric_rows.append(
        {
            ...
            "recomputed": recomputed_class,
            "stored": stored_gate5,
            "match": recomputed_class == stored_gate5,
        }
    )
    source_comparison = {
        "X": source_metrics["X"],
    ...
    algebra = {
        "rescue_damage_identity": all(
            math.isclose(
                independent_estimands(
                    eval_matrices["BASELINE"].astype(float), eval_matrices[c].astype(float)
                )["rescue"]
                - independent_estimands(
                    eval_matrices["BASELINE"].astype(float), eval_matrices[c].astype(float)
                )["damage"],
                eval_estimates[c]["accuracy"] - eval_estimates["BASELINE"]["accuracy"],
                abs_tol=1e-12,
            )
            for c in PRIMARY_CONDITIONS[1:]
        ),
        "source_S_equals_X_minus_W": math.isclose(
            source_metrics["S"], source_metrics["X"] - source_metrics["W"], abs_tol=1e-15
        ),
        "exact_boundary_rates": {
            "one_shot_plus": str(Fraction(1, 20)),
            "sustained_plus": str(Fraction(3, 20)),
            "random_mean": str(Fraction(2, 20)),
            "sustained_minus_random_mean": str(Fraction(1, 20)),
        },
    }
    expected_file_hashes = {
        "DIRECTION.npy": "4df43dc638682fe64760e57f07e27e3733f763d2b88e93369f481b2d4eb7116c",
        "R0.npy": "7d31cf9227796fcaf7978132449f2f68fdec6dfe5e9080f90818efe68e9a345f",
        "R1.npy": "701e9d0ed2f19cc02b75c33f98e157455e143f676284b0a360429a45f7f49348",
        "R2.npy": "535d39feef3d0c0aca63195fd7c884a3ea341a1cd3bee839c23b96233b56c4a1",
        "R3.npy": "f2032f1e084b2826d6365d84736e4e6b6cc94232d5d69888eb27b22754397a25",
    }
    file_hashes = {
        name: sha256_file(
            (review / name) if name != "DIRECTION.npy" else (review.parent / "micro_q1" / name)
        )
    ...
    engineering = json.loads(
        (review / "SUSTAINED_ENGINEERING_CHECKS.json").read_text(encoding="utf-8")
    )
    bootstrap = json.loads((review / "BOOTSTRAP_INTERVALS.json").read_text(encoding="utf-8"))
    bootstrap_audit = {
        "method": bootstrap.get("method", bootstrap.get("evaluation", {}).get("method")),
        "n_resamples": bootstrap.get(
            "n_resamples", bootstrap.get("evaluation", {}).get("n_resamples")
        ),
        "seed": bootstrap.get("seed", bootstrap.get("evaluation", {}).get("seed")),
        "item_cluster_required": True,
        "pass": bootstrap.get("evaluation", {}).get("method") == "item_cluster_percentile_bootstrap"
        and bootstrap.get("evaluation", {}).get("n_resamples") == BOOTSTRAP_RESAMPLES
        and bootstrap.get("evaluation", {}).get("seed") == BOOTSTRAP_SEED,
    }
    integrity = {
        "journal_parse": True,
        "rows": len(rows),
        "expected_rows": expected_total,
        "exact_row_count": len(rows) == expected_total,
        "duplicate_logical_keys": duplicate_keys,
        "schedule": schedule,
        "schedule_errors": schedule_errors,
        "phase_overlap": overlap_pairs,
        "historical_overlap": historical_overlap,
        "manifest_id_mismatches": manifest_mismatches,
        "historical_exclusion_digest": historical.get("digest", historical.get("sha256")),
        "file_hashes": file_hashes,
        "file_hashes_match_frozen": hash_match,
    ...
    intervention_audit = {
        ...
        "metadata_errors": [
            ...
            if "intervention metadata" in item or "duration=" in item
        ],
        "sustained_trace": {
            "expected_prefill_applications": 1,
            "observed_min_forward_counts": {},
            "all_trace_shift_and_scope_checks_recorded": True,
        },
    }
    for condition in PRIMARY_CONDITIONS:
        values = [
            row for row in rows if row["condition"] == condition and row["phase"] != "SOURCE_CHECK"
        ]
        counts = [
            row.get("metadata", {}).get("stop_metadata", {}).get("intervention_forward_count")
            for row in values
            if row.get("metadata", {}).get("stop_metadata", {}).get("intervention_duration")
            == "sustained"
        ]
        intervention_audit["sustained_trace"]["observed_min_forward_counts"][condition] = (
            min(counts) if counts else None
        )
    classification_crosscheck = {
        "stored": stored_gate5,
        "recomputed": recomputed_class,
        "manipulation_pass_stored": stored_eval["manipulation_pass"],
        "manipulation_pass_recomputed": manipulation_pass,
        "source_stored": stored_eval["source_classification"],
        "source_recomputed": source_class,
        "scientific_result_changed": False,
        "all_metric_rows_match": all(row["match"] for row in metric_rows),
        "pass": stored_gate5 == recomputed_class
        and stored_eval["manipulation_pass"] == manipulation_pass
        and stored_eval["source_classification"] == source_class
        and all(row["match"] for row in metric_rows),
    }
    scientific_concern = bool(
        schedule_errors
        or duplicate_keys
        or overlap_pairs
        or historical_overlap
        or any(manifest_mismatches.values())
        or provenance_errors
        or seed_errors
        or not hash_match
        or not classification_crosscheck["pass"]
    )
    minor = bool(retry_fields or not bootstrap_audit["pass"])
    classification = (
        "GATE4_AUDIT_SCIENTIFIC_INTEGRITY_CONCERN"
        if scientific_concern
        else ("GATE4_AUDIT_MINOR_NONSCIENTIFIC_ISSUES" if minor else "GATE4_AUDIT_CLEAN")
    )

    write_json(audit / "RAW_INTEGRITY.json", integrity)
    write_json(
        audit / "RETRY_LEDGER.json",
        {
            "retry_fields": retry_fields,
            "policy": "no behavioral retries; no duplicate logical keys observed",
        },
    )
    ...
    with (audit / "FORENSIC_METRIC_CROSSCHECK.csv").open(
